...
- **2026-08-14**: proxy (maximum) bidding live-verified end-to-end against the real database and, separately, through the real buyer web UI -- raise-only enforcement (including strict-inequality on equal maxima and idempotent-retry-of-rejection), equal-maximum tie-break by earlier `priority_at`, and automatic proxy counter-response to a competing manual bid all behaved exactly as `bidding.service.ts` implements them. Full detail, including two real dev-environment config gaps found along the way (CORS `WEB_BASE_URL` allowlist, a stale production `.next` build masking dynamic lot routes) and confirmation that the contract-documented `DELETE /lots/:lotId/proxy-bid` and `GET /lots/:lotId/my-proxy-bid` routes have no server implementation, in `docs/current-state.md`'s "Proxy (maximum) bidding live-verified" section.
- **2026-08-15**: closed the proxy REST route drift noted above: implemented `GET /api/v1/lots/:lotId/my-proxy-bid` and `DELETE /api/v1/lots/:lotId/proxy-bid`. `GET` exposes only the authenticated bidder's active maximum plus latest public lot state; `DELETE` returns an explicit non-retryable `VALIDATION_FAILED` command rejection for MVP instead of 404. Updated `docs/api-contracts.md`, `packages/contracts/src/rest.ts`, and regenerated OpenAPI.
