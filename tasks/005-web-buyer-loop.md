...
## Real MVP-blocker fixes, verified against a live backend (added 2026-08-14)

A local PostgreSQL instance became available for the first time in this engagement (see `docs/decisions-log.md` DEC-020). This let every piece of this task's client work be exercised against a real API for the first time -- and surfaced two real, previously-invisible bugs specific to this app (a third, server-side lifecycle-scheduler gap that blocked this task indirectly is covered in `docs/current-state.md`'s "MVP blockers closed" section and `tasks/004-bidding-engine.md`).

- **Fixed a real, confirmed-blocking bug**: `apps/web/app/[locale]/lots/[lotId]/page.tsx` hard-locked to three fixed dummy lot UUIDs via `export const dynamicParams = false`, so any real lot ID 404'd -- confirmed live before the fix. This made the entire `BidPanelShell` built by this task unreachable for any real lot regardless of whether the underlying engine worked. Next.js requires `dynamicParams` to be a static literal (rejects computed expressions) and separately forbids `dynamicParams: true` under `output: "export"`, so the same line can't serve both `next dev`/a real server deployment and the Cloudflare static-export preview. Fixed by checking in `dynamicParams = true` and adding `apps/web/scripts/build-cloudflare.mjs`, a wrapper that temporarily flips the literal to `false` only around the Cloudflare build and restores it afterward, so local `build:cloudflare` runs never leave the working tree modified.
- **Fixed a real Socket.IO event-shape bug**, found immediately once the above fix made a real lot reachable and a real bid was placed through the actual UI for the first time: `BidPanelShell`'s `onBidAccepted` handler read `event.data.currentBid`, matching `docs/api-contracts.md` §9's documented `LotEventEnvelope<Name, Data>` wrapper -- but the real server emits every outbox event payload flat and unwrapped, so `event.data` was always `undefined`. This crashed with a real, visible console error on every accepted bid (the bid itself still succeeded via the REST fallback path, so it wasn't silently wrong, just noisy/unstable). Fixed by flattening `BidAcceptedEvent`/`AuctionStateChangedEvent`/`AuctionExtendedEvent`/`ReserveStatusChangedEvent` in `lot-socket.ts` to match the real server shape; see `docs/decisions-log.md` DEC-022 for full detail, including that `auction:extended`/`reserve:status-changed` are never actually emitted by any server code today.
- **First-ever complete live verification of this task's core promise**: with a real lot auto-opened by the new backend scheduler, navigated to its real detail page (rendered correctly, not a 404), and placed two real bids through the actual browser UI (mode selection, terms checkbox, confirm dialog, real REST submission) -- both accepted and correctly recorded in `bid_ledger` at increasing sequence numbers, the second one confirming the socket fix left zero console errors.
- Not yet exercised live even after this round: Socket.IO proxy/maximum bidding, outbid/rejected/closed bid states, soft-close extension, and reconnect/gap-resync under a real network drop -- only the manual-bid REST path has been proven end-to-end against a real backend. Also noted but not investigated: the "Current bid" display didn't visibly refresh immediately after the buyer's own second bid, even though the underlying ledger/lot state were correct -- likely a missing client-side refetch/state update after a successful submit.
