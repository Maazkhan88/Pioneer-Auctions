...
- The buyer web app's lot detail page (`apps/web/app/[locale]/lots/[lotId]/page.tsx`) hard-locks to three fixed dummy lot UUIDs via `export const dynamicParams = false` and a static `previewLotIds` list -- confirmed by a real 404 when navigating to a real lot ID from the live database. This was a deliberate choice for the Cloudflare static-export preview deployment, but it means the entire bid-panel-shell UI (manual/proxy bidding, the socket connection, all of it) is currently unreachable for any lot that isn't one of those three IDs. The underlying bid-placement REST/Socket.IO engine itself works correctly against real lots (see below) -- this is specifically a routing/page-generation gap in the buyer web app's UI layer, not the bidding engine.
...
- **Not yet exercised live**: Socket.IO bidding (`BiddingGateway`), proxy/maximum bidding, outbid/rejected/closed bid states, soft-close extension, and the buyer web app's actual UI (blocked by the `dynamicParams = false` routing gap noted above -- the underlying REST engine was verified directly instead).
...
With a real local Postgres now available, the two biggest remaining verification gaps are: (1) exercise the Socket.IO bidding gateway and proxy bidding live (REST manual bidding is now verified; sockets and proxy bids are not), and (2) fix the buyer web app's `dynamicParams = false` lot-detail routing gap so real lots can actually be browsed and bid on through the UI, not just via direct API calls -- until that's fixed, the buyer bid-panel-shell UI built across this whole engagement remains unreachable for any real lot. After that: a dedicated single-lot/snapshot REST endpoint to replace the current "refetch the whole public lot list" approach in `fetchAuthoritativeLotState`, and diagnosing the Turbopack/`@pioneer/contracts` bundling gap (DEC-019).
